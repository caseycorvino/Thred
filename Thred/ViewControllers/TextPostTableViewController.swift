//
//  TextPostTableViewController.swift
//  Thred
//
//  Created by Thred. on 12/8/17.
//  Copyright © 2017 corvino. All rights reserved.
//

import UIKit

class TextPostTableViewController: UITableViewController, UITextFieldDelegate  {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if(indexPath.row == 0){
            if let cell = tableView.dequeueReusableCell(withIdentifier: "TextPost", for: indexPath) as? TextPostTableViewCell{
                if(clickedPost != nil){
    ...
    @objc func likeButtonClicked(sender: UIButton){
        ...
        postServices.likePost(id: clickedPost!.objectId!) { (likeStr: String, likes:[String])  in
            if let cell = self.tableView.cellForRow(at: [0,0]) as? TextPostTableViewCell {
